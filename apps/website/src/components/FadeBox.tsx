...
const fadePropsDefault = ['animation', 'duration', 'offset', 'disabled'];

function extractFadeProps(className: string, fadeProps: string[]) {
    if (className.charAt(className.indexOf('fade') + 4) == '?') {
        const fadePropsString = className.substring(
            className.indexOf('fade') + 5,
            className.indexOf(';') == -1 ? className.length : className.indexOf(';'),
        );
        ...
        const processedFadePropsArray = rawFadePropsArray
            .filter(fadeProp => fadeProp != null)
            .map(fadeProp => {
...
export default function FadeBox({ children, all, component = <></>, ...props }: FadeBoxProps) {
    ...
    const fadeChildren = Children.map(children, child => {
        if (
            ...
            (child.props?.className?.includes('fade') || all) &&
            !child.props?.className?.includes('skip') &&
            animations
        ) {
